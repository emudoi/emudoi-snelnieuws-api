...
class NotificationServiceSpec
  ...
  "dispatch" should {
    ...
    // 2026-05-23 regression test. Yesterday's temp TARGET_LANGUAGES=nl
    // override produced top_summaries with notification_messages keyed
    // only on `nl`. All 35 production subscribers were
    // notification_language='en' (the schema default). Pre-fix the
    // dispatcher iterated messages-by-language: for each lang in
    // messages (`nl` only), it looked up subscribers in that bucket
    // (none) → silently skipped the whole dispatch → `sent_count=0`
    // for ~16h. New behaviour iterates SUBSCRIBER buckets and falls
    // back to the `en` message when the bucket's exact language is
    // not in the top_summary; this test exercises the fallback path
    // for an English-default subscriber against a top_summary that
    // has BOTH `en` and `nl` keys.
    "english-default subscriber receives the en message when top_summary has en + nl" in {
      requireDb()
      val prodStub = new StubApnsMessagingService(acceptAll = true)
      val service  = newService(apnsProd = Some(prodStub))
      service.subscribe(
        SubscribeRequest(
          deviceId    = "ns-spec-en-default",
          apnsToken   = "ns-spec-en-default-token",
          frequency   = 2,
          environment = "production"
        )
      ) shouldBe a[Right[_, _]]
      // language defaults to 'en' (schema default).
      seedTopSummary(messages = Map(
        "en" -> "English clickbait headline",
        "nl" -> "Nederlands clickbait kop"
      ))
      service.dispatch(frequency = Some(2), environment = "production") match {
        case Right(DispatchOutcome.Sent(resp)) =>
          resp.sent should be >= 1
      ...
      // The English subscriber must have received the English title.
      val sentTitles = prodStub.batches.map(_.title).toSet
      sentTitles should contain("English clickbait headline")
      sentTitles should not contain "Nederlands clickbait kop"
    }

    // Hardening test for the exact 2026-05-22 incident: top_summary
    // has ONLY `nl`. Subscriber is 'en' (default). Pre-fix the
    // dispatch sent 0 notifications (no `en` key → message-by-lang
    // loop never picked up the en bucket). With sendByLanguage now
    // iterating subscriber buckets + falling back to en when present,
    // a top_summary that has only `nl` and no `en` falls into the
    // "no fallback available" branch and STILL sends 0 — but logs a
    // warning rather than silently no-oping. This test pins the
    // dispatch behaviour AND verifies the path doesn't crash.
    "no-en, only-nl top_summary still sends 0 to en subscribers (no fallback available)" in {
      requireDb()
      val prodStub = new StubApnsMessagingService(acceptAll = true)
      val service  = newService(apnsProd = Some(prodStub))
      service.subscribe(
        SubscribeRequest(
          deviceId    = "ns-spec-nofallback",
          apnsToken   = "ns-spec-nofallback-token",
          frequency   = 1,
          environment = "production"
        )
      ) shouldBe a[Right[_, _]]
      seedTopSummary(messages = Map("nl" -> "Nederlands only"))
      service.dispatch(frequency = Some(1), environment = "production") match {
        case Right(DispatchOutcome.Sent(resp)) =>
          // No en key in messages, en-bucket has no fallback → 0 sent.
          // This is the documented "no message we can deliver" case.
          resp.sent shouldBe 0
        case other => fail(s"Expected Sent, got: $other")
      }
      prodStub.batches shouldBe empty
